...
class VerbosityReducer:
    ...
    def reduce_assistant_verbosity(self, text: str, max_words: int = 150) -> str:
        """
        Make assistant messages more chat-like by removing verbose patterns.

        Args:
            text: The assistant message to process
            max_words: Maximum word count (default 150, ~5 sentences)
        ...
        # 6. Remove overly empathetic/verbose openings
    ...
    def check_message_quality(self, text: str, role: str = "user") -> dict:
        ...
        if role == "user":
            ...
            if word_count > 150:
                warnings.append(f"Too long: {word_count} words (target: <150)")
