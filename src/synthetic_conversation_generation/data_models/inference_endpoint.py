...
class InferenceEndpoint:
    ...
    def get_assistant_message(self, conversation: Conversation, system_prompt: str = None) -> Message:
        """
        ...
        Args:
            ...
            system_prompt: Optional system prompt to prepend (e.g., assistant description)
        """
        ...
        # Add system prompt if provided (CRITICAL for constraining assistant behavior)
        if system_prompt:
            messages.append({'role': 'system', 'content': system_prompt})

        # Add conversation history
        messages.extend([{'role': msg.role.name, 'content': msg.content} for msg in conversation.messages])
