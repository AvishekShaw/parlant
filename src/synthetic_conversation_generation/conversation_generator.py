...
from synthetic_conversation_generation.verbosity_reducer import VerbosityReducer
...
class ConversationGenerator:
    ...
    def __init__(self, model_provider: ModelProvider, model_id: str, assistant_endpoint: InferenceEndpoint, assistant: Assistant, user_persona: CharacterCard, max_conversation_turns: int, conversation_completion_query_model_id: str):
        ...
        self.verbosity_reducer = VerbosityReducer()
    ...
    def generate_conversation(self, conversation_id: str) -> Conversation:
        ...
        for i in range(self.max_conversation_turns):
            ...
            # Apply verbosity reduction to user message
            original_content = user_message.content
            user_message.content = self.verbosity_reducer.reduce_user_verbosity(user_message.content)

            # Check quality and log warnings
            quality = self.verbosity_reducer.check_message_quality(user_message.content, role="user")
            if not quality["is_acceptable"]:
                logger.warning(f"User message quality issues: {quality['warnings']}")

            conversation.messages.append(user_message)

            # Generate assistant response with system prompt from assistant definition
            assistant_message = self.assistant_endpoint.get_assistant_message(
                conversation,
                system_prompt=self.assistant.description
            )

            # Apply verbosity reduction to assistant message
            assistant_message.content = self.verbosity_reducer.reduce_assistant_verbosity(assistant_message.content)

            # Check quality and log warnings
            quality = self.verbosity_reducer.check_message_quality(assistant_message.content, role="assistant")
            if not quality["is_acceptable"]:
                logger.warning(f"Assistant message quality issues: {quality['warnings']}")
